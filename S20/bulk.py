...
def distance_to_nearest_halo(data):
    ...
    return np.array(distances)/0.6776942783267969
...
def distance_to_threshold_host(data):
    distances = []
    hostrvirs = []
    for i in range(len(data)):
        s = data['sim'].tolist()[i]
        mvir = data['mass'].tolist()[i]
        mstar = data['M_star'].tolist()[i]
        
        if s=='h148' or s=='h229' or s=='h242' or s=='h329': # if sat simulation, find distance to halo 1

            halocoords = np.array([data['Xc'].tolist()[i],data['Yc'].tolist()[i],data['Zc'].tolist()[i]])
            
            rvirs = np.array(data['Rvir'],dtype=float)
            mstars = np.array(data['M_star'],dtype=float)

            x = np.array(data['Xc'],dtype=float)[mstars > 10*mstar]
            y = np.array(data['Yc'],dtype=float)[mstars > 10*mstar]
            z = np.array(data['Zc'],dtype=float)[mstars > 10*mstar]
            try:
                test = x[0]
            except:
                print('no halo with stellar mass 10 times greater than this halo...')
                distances.append(0.)
                hostrvirs.append(1.)
                continue
            ...
            rvir = rvirs[np.argmin(np.sqrt(np.sum((halocoords-coords)**2,axis=1)))]*0.6776942783267969
            dist = np.min(np.sqrt(np.sum((halocoords - coords)**2, axis=1)))*0.6776942783267969

            distances.append(dist)
            hostrvirs.append(rvir)
            ...
            coords = coords[np.array(coords.mass,dtype=float) > 10*mvir]
